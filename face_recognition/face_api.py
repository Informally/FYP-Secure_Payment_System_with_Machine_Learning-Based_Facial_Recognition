            ...
            if len(rects) > 0:
                ...
                if ear < threshold:
                    ...
                    if counter >= consecutive_frames:
                        blink_counter += 1
                        logger.info(f"Blink detected at frame {i}, EAR: {ear:.3f}")
                    counter = 0
        except Exception as e:
    ...
    if blink_counter == 0 and len(ear_history) > 5:
        for i in range(2, len(ear_history) - 2):
            ...
            if (ear_history[i] < ear_history[i-1] and 
                ...
                blink_counter += 1
                logger.info(f"Blink detected using pattern analysis, EAR: {ear_history[i]:.3f}")
    
    logger.info(f"Blink detection: {blink_counter} blinks, Avg EAR: {avg_ear:.3f}, EAR StdDev: {ear_std:.3f}")
...
def detect_smile(frame_sequence):
    ...
    logger.info(f"Smile detection: {smile_detected}, Confidence: {smile_confidence:.2f}")
...
def detect_head_movement(frame_sequence, movement_type='horizontal'):
    ...
    if movement_type == 'horizontal':
        ...
        logger.info(f"Horizontal movement: {x_movement}, Range: {x_range}")
        ...
        logger.info(f"Vertical movement: {y_movement}, Range: {y_range}")
...
# Modify verify_liveness_multimethod to have stricter thresholds
def verify_liveness_multimethod(frame_sequence):
    """
    Comprehensive liveness detection combining multiple methods with stricter thresholds
    """
    ...
    # Calculate weighted final score - INCREASED WEIGHTS for critical components
    weights = {
        'blink': 0.10,                # Reduced reliance on blink
        'head_movement': 0.15,
        'optical_flow': 0.30,         # Increased weight on movement analysis
        'texture': 0.15,
        'frequency': 0.15,
        'deep_learning': 0.25,
        'deepfake': 0.15
    ...
    # STRICTER threshold - increased from 0.6 to 0.7
    threshold = 0.6
    is_live = final_score >= threshold
    
    # Additional requirement: must pass at least 70% of tests
    passed_tests = sum(1 for result in methods_results.values() if result)
    pass_ratio = passed_tests / len(methods_results)
    
    if pass_ratio < 0.6:
        is_live = False  # Fail if less than 70% of tests pass
    ...
    logger.info(f"Final liveness assessment: {is_live}, score: {final_score:.2f}, pass ratio: {pass_ratio:.2f}")
    
    return is_live, final_score, details

# Liveness detection: Main function that combines all methods
def verify_liveness(frame_sequence):
    # For backward compatibility - use the new multimethod approach
    is_live, score, details = verify_liveness_multimethod(frame_sequence)
...
def get_random_challenge():
    try:
        ...
        # Return the challenge and session ID
        return jsonify({
            "status": "success",
        ...
        logger.error(f"Error generating random challenge: {e}")
        return jsonify({
            "status": "failed",
...
# Verify liveness challenge - ENHANCED with multimethod approach
...
def verify_liveness_challenge():
    try:
        ...
        # Get frames and challenge type
        frames = active_challenges[session_id]['frames']
        challenge_type = active_challenges[session_id]['type']
        
        # Check if enough frames received
        if len(frames) < 4:  # Reduced minimum required frames
            return jsonify({
                "status": "failed", 
        ...
        if challenge_type == 'blink':
            # Check for blinks with adaptive threshold
            blink_count, ear_values = detect_blinks(frames)
            is_live = blink_count >= 1
            liveness_score = min(blink_count / 2.0, 1.0)
            details = f"Blinks detected: {blink_count}"
            ...
            liveness_score = smile_confidence
            details = f"Smile detected: {smile_detected}, confidence: {smile_confidence:.2f}"
        
        elif challenge_type in ['head_movement', 'head_left', 'head_right']:
            # Check for head movement
            movement_type = 'horizontal'
            movement_detected, movement_range = detect_head_movement(frames, movement_type)
            is_live = movement_detected
            liveness_score = min(movement_range / 50.0, 1.0)
            details = f"Head movement: {movement_detected}, Range: {movement_range}"
        
        elif challenge_type in ['head_up', 'head_down']:
            # Check for vertical head movement
            movement_detected, movement_range = detect_head_movement(frames, 'vertical')
            is_live = movement_detected
            liveness_score = min(movement_range / 50.0, 1.0)
            details = f"Vertical head movement: {movement_detected}, Range: {movement_range}"
        
        else:
            # Use all methods for verification
            is_live, liveness_score, details = verify_liveness_multimethod(frames)
        
        # Mark challenge as completed
        active_challenges[session_id]['completed'] = True
        active_challenges[session_id]['liveness_verified'] = is_live
        active_challenges[session_id]['liveness_score'] = liveness_score
...
def register():
    try:
        ...
        if liveness_session_id:
            if liveness_session_id in active_challenges and active_challenges[liveness_session_id]['completed']:
                # Use results from completed liveness challenge
                is_live = active_challenges[liveness_session_id].get('liveness_verified', False)
                if not is_live:
                    return jsonify({
                        "status": "failed", 
                        "message": "Liveness verification failed. Please try again with a real person."
                    })
                
                # Get liveness score if available
                liveness_score = active_challenges[liveness_session_id].get('liveness_score', 0.5)
            else:
                # Liveness challenge not completed or invalid
                logger.warning(f"Invalid or incomplete liveness session: {liveness_session_id}")
        ...
        # Continuing from the register endpoint code
        if existing_count > 0:
            logger.warning(f"User {user_id} already exists")
            log_security_event(user_id, "register", "failed", client_ip=client_ip)
            return jsonify({"status": "failed", "message": "User already exists."})
    ...
    try:
        ...
        if liveness_session_id:
            if liveness_session_id in active_challenges and active_challenges[liveness_session_id]['completed']:
                ...
                is_live = active_challenges[liveness_session_id].get('liveness_verified', False)
                liveness_score = active_challenges[liveness_session_id].get('liveness_score', 0.0)
                
                # Require liveness for authentication
                if not is_live and liveness_score < 0.5:
                    return jsonify({
                        "status": "failed", 
                        "message": "Liveness verification failed. Authentication requires a live person."
                    })
            else:
                # Invalid or incomplete liveness challenge
                logger.warning(f"Invalid or incomplete liveness session: {liveness_session_id}")
...
# Cleanup expired liveness challenges periodically
def cleanup_expired_challenges():
    current_time = datetime.now()
    expired_sessions = []
    
    for session_id, challenge in active_challenges.items():
        # Expire after 5 minutes
        if (current_time - challenge['created_at']).total_seconds() > 300:
    ...
    if expired_sessions:
        logger.info(f"Cleaned up {len(expired_sessions)} expired liveness challenges")

# Enhanced combined authentication endpoint
@app.route('/authenticate', methods=['POST'])
def authenticate():
    """
    Combined endpoint for liveness detection and recognition in a single step.
    Expects a main frame and several additional frames to perform liveness check
    while also doing the facial recognition immediately.
    """
    try:
        data = request.json
        client_ip = request.remote_addr
        ...
        logger.info(f"Number of frames received: {len(additional_frames) + 1}")
        ...
        frames = [main_image]
        for frame_base64 in additional_frames:
            frame = base64_to_image(frame_base64)
        ...
        # NEW: Check for static image (all frames identical)
        ...
        is_live, liveness_score, liveness_details = verify_liveness_multimethod(frames)
        
        # NEW: Specific challenge verification based on challenge_type
        if challenge_type == 'blink':
            ...
            smile_detected, _ = detect_smile(frames)
            ...
            movement_detected, _ = detect_head_movement(frames, 'horizontal')
            ...
            movement_detected, _ = detect_head_movement(frames, 'vertical')
            if not movement_detected:
        ...
        # NEW: Mandatory check for natural movement via optical flow
        natural_movement, _, flow_details = analyze_optical_flow(frames)
        if not natural_movement:
        ...
        if is_live:
            ...
            if recognized_user:
                # Success - user recognized
                logger.info(f"User authenticated: {recognized_user}, similarity: {similarity:.4f}, matches: {match_count}")
                log_security_event(recognized_user, "login", "success", 
                                  similarity_score=similarity, client_ip=client_ip,
                                  liveness_score=liveness_score)
            ...
            logger.warning(f"Liveness check failed. Details: {liveness_details}")
            log_security_event(user_id if user_id else "unknown", "login", "failed", 
...
# Advanced liveness verification endpoint with anti-deepfake
@app.route('/verify_advanced_liveness', methods=['POST'])
def verify_advanced_liveness():
    """
    Advanced liveness detection endpoint with anti-deepfake capabilities.
    This endpoint combines multiple methods for more robust verification.
    """
    try:
        data = request.json
        ...
        # Process frames
        frames = []
        if 'frames_base64' in data and isinstance(data['frames_base64'], list):
            for frame_base64 in data['frames_base64']:
                frame = base64_to_image(frame_base64)
                if frame is not None:
                    frames.append(frame)
        ...
        if len(frames) < 4:
            return jsonify({
                "status": "failed",
                "message": f"Not enough valid frames. Received: {len(frames)}, need at least 4."
            })
        ...
        # 1. Check for blinks
        blink_count, _ = detect_blinks(frames)
        results['blink_detection'] = {
            'passed': blink_count >= 1,
            'details': f"Detected {blink_count} blinks",
            'score': min(blink_count / 2.0, 1.0)
        }
        
        # 2. Check for smile/facial expression
        smile_detected, smile_confidence = detect_smile(frames)
        results['facial_expression'] = {
            'passed': smile_detected,
            'details': f"Smile detected: {smile_detected}, confidence: {smile_confidence:.2f}",
            'score': smile_confidence
        }
        
        # 3. Check for head movement
        head_move, move_range = detect_head_movement(frames)
        results['head_movement'] = {
            'passed': head_move,
            'details': f"Head movement detected: {head_move}, range: {move_range}",
            'score': min(move_range / 40.0, 1.0)
        }
        
        # 4. Analyze optical flow for micro-movements
        natural_movement, flow_score, flow_details = analyze_optical_flow(frames)
        results['optical_flow'] = {
            'passed': natural_movement,
        ...
        real_texture, texture_score = detect_spoofing(middle_frame)
        results['texture_analysis'] = {
            'passed': real_texture,
            'details': f"Texture analysis: {'Real' if real_texture else 'Fake'}, score: {texture_score:.2f}",
            'score': texture_score
        }
        ...
        results['frequency_analysis'] = {
            'passed': real_freq,
            'details': f"Frequency analysis: {'Real' if real_freq else 'Fake'}, score: {freq_score:.2f}",
            'score': freq_score
        }
        ...
        results['deepfake_detection'] = {
            'passed': not_deepfake,
            'details': f"Deepfake detection: {'Not Deepfake' if not_deepfake else 'Potential Deepfake'}, score: {deepfake_score:.2f}",
            'score': deepfake_score
        }
        
        # 8. Deep learning (if model available)
        if os.path.exists(LIVENESS_MODEL_PATH):
            dl_real, dl_score = deep_learning_liveness(frames)
            results['deep_learning'] = {
                'passed': dl_real,
                'details': f"Deep learning: {'Real' if dl_real else 'Fake'}, score: {dl_score:.2f}",
                'score': dl_score
            }
        
        # Calculate final score (weighted average)
        weights = {
            'blink_detection': 0.15,
        ...
        for key, weight in weights.items():
            if key in results and 'score' in results[key]:
                final_score += results[key]['score'] * weight
                total_weight += weight
        ...
        # Final decision (more tests must pass and good final score)
        is_live = (passed_tests >= total_tests * 0.6) and (final_score >= 0.65)
        
        # Return detailed results
        return jsonify({
            "status": "success" if is_live else "failed",
            "liveness_verified": is_live,
            "liveness_score": float(final_score),
            "passed_tests": passed_tests,
            "total_tests": total_tests,
            "test_results": results,
            "message": f"Advanced liveness {'verified' if is_live else 'check failed'} with score {final_score:.2f}"
        })
        
    except Exception as e:
        logger.error(f"Advanced liveness verification error: {str(e)}", exc_info=True)
        return jsonify({
            "status": "failed",
            "message": f"Advanced liveness verification error: {str(e)}"
        })

# Function to train a custom liveness detection model
@app.route('/train_liveness_model', methods=['POST'])
def train_liveness_model_endpoint():
    """
    Train a custom liveness detection model using provided real and fake face samples.
    Requires a dataset of labeled face images.
    """
    try:
        data = request.json
        
        # Check if dataset paths are provided
        real_faces_dir = data.get('real_faces_dir')
        fake_faces_dir = data.get('fake_faces_dir')
        
        if not real_faces_dir or not fake_faces_dir:
            return jsonify({
                "status": "failed",
                "message": "Please provide paths to real and fake face image directories."
            })
        
        # Check if directories exist
        if not os.path.exists(real_faces_dir) or not os.path.exists(fake_faces_dir):
            return jsonify({
                "status": "failed",
                "message": "One or both directories do not exist."
            })
        ...
        def train_model_thread():
            try:
                ...
                real_images, real_labels = load_images(real_faces_dir, 1)  # 1 for real
                fake_images, fake_labels = load_images(fake_faces_dir, 0)  # 0 for fake
                ...
                for epoch in range(num_epochs):
                    ...
                    logger.info(f"Epoch {epoch+1}/{num_epochs}, Loss: {running_loss/len(dataloader):.4f}, Accuracy: {accuracy:.2f}%")
                
                # Save the model
                torch.save(model.state_dict(), LIVENESS_MODEL_PATH)
                logger.info(f"Model saved to {LIVENESS_MODEL_PATH}")
                
            except Exception as e:
                logger.error(f"Error training liveness model: {str(e)}", exc_info=True)
...
# Add a new endpoint for multi-challenge liveness verification
@app.route('/verify_multi_challenge', methods=['POST'])
def verify_multi_challenge():
    """
    Perform multiple liveness challenges in sequence for enhanced security.
    This is used for high-security scenarios where a single challenge isn't enough.
    """
    try:
        data = request.json
        ...
        # Define a sequence of challenges
        challenge_sequence = [
            {"type": "blink", "instruction": "Please blink 2-3 times naturally"},
            {"type": "smile", "instruction": "Please smile naturally"},
            {"type": "head_left", "instruction": "Please turn your head left and back to center"},
            {"type": "head_right", "instruction": "Please turn your head right and back to center"}
        ]
        
        # Randomize the order for additional security
        ...
        logger.error(f"Error creating multi-challenge: {str(e)}")
        return jsonify({
            "status": "failed",
...
def submit_multi_challenge_frames():
    try:
        data = request.json
        session_id = data.get('session_id')
        frames_base64 = data.get('frames', [])
        
        if not session_id or not frames_base64:
            return jsonify({"status": "failed", "message": "Missing session_id or frames"})
        
        if session_id not in active_challenges or 'sequence' not in active_challenges[session_id]:
            return jsonify({"status": "failed", "message": "Invalid or expired multi-challenge session"})
        
        # Get challenge session data
        challenge_data = active_challenges[session_id]
        
        # Check if challenge is already completed
        if challenge_data['completed']:
            return jsonify({"status": "failed", "message": "Multi-challenge already completed"})
        ...
        if len(frames) < 4:  # Need at least 4 frames
            return jsonify({
                "status": "failed", 
                "message": f"Not enough valid frames for verification. Received: {len(frames)}, need at least 4."
            })
        
        # Get current challenge type
        current_index = challenge_data['current_index']
        challenge_type = challenge_data['sequence'][current_index]['type']
        ...
        if challenge_type == 'blink':
            ...
            is_valid = movement_detected
            score = min(movement_range / 50.0, 1.0)
            details = f"Head movement: {movement_detected}, Range: {movement_range}"
        ...
        # Store result for this challenge
        challenge_data['results'].append({
            'type': challenge_type,
            'passed': is_valid,
            'score': score,
            'details': details
        ...
        if current_index + 1 < len(challenge_data['sequence']):
            # More challenges to go
            challenge_data['current_index'] += 1
            next_challenge = challenge_data['sequence'][current_index + 1]
            
            return jsonify({
                "status": "success",
                "challenge_completed": is_valid,
                "challenge_score": score,
                "challenge_details": details,
                "next_challenge_type": next_challenge["type"],
                "next_instruction": next_challenge["instruction"],
                "total_challenges": len(challenge_data['sequence']),
                "current_challenge": current_index + 2  # 1-based for display
            })
            
        else:
            # All challenges completed, calculate final result
            passed_challenges = sum(1 for result in challenge_data['results'] if result['passed'])
            total_challenges = len(challenge_data['results'])
            
            total_score = sum(result['score'] for result in challenge_data['results'])
            avg_score = total_score / total_challenges if total_challenges > 0 else 0
            
            # Overall verification is successful if most challenges passed and good average score
            final_success = (passed_challenges / total_challenges >= 0.7) and (avg_score >= 0.65)
            
            # Mark session as completed
            challenge_data['completed'] = True
            challenge_data['liveness_verified'] = final_success
            challenge_data['liveness_score'] = avg_score
            
            return jsonify({
                "status": "success",
                "multi_challenge_completed": True,
                "liveness_verified": final_success,
                "liveness_score": avg_score,
                "passed_challenges": passed_challenges,
                "total_challenges": total_challenges,
                "challenge_results": challenge_data['results']
            })
        
    except Exception as e:
        logger.error(f"Error in multi-challenge verification: {str(e)}")
        return jsonify({"status": "failed", "message": f"Error in multi-challenge verification: {str(e)}"})
