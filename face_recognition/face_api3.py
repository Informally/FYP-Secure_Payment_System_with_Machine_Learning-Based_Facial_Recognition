...
def verify_facial_expression(frame_sequence, expression_type):
    if expression_type == 'smile':
        ...
        # Cross-check with optical flow to ensure natural movement
...
def analyze_optical_flow(frame_sequence):
    ...
    # Stricter conditions for natural movement
...
def frequency_domain_liveness(face_image):
    try:
        ...
        mid_freq_mask = ((rows//10)**2 < (y - center_row)**2 + (x - center_col)**2 <= (rows//5)**2)
        ...
        # Stricter ratio for real faces
        freq_ratio = high_freq_energy / (low_freq_energy + 1e-10)
        is_real = freq_ratio > 0.25 and high_freq_energy > 0.3
        
        # Check for screen-specific frequency patterns (e.g., refresh rate artifacts)
        ...
        is_real = is_real and (h_variance < 8000 and v_variance < 8000)  # Screens have high variance
...
def detect_spoofing(face_image):
    try:
        ...
        # Enhanced LBP with multiple radii for better texture discrimination
        lbp_scores = []
        for radius in [1, 2, 3]:
...
def detect_deepfake(face_image):
    try:
        ...
        # Define frequency bands
        r1, r2, r3 = rows // 16, rows // 8, rows // 4
        ...
        # Calculate energy
        total_energy = np.sum(magnitude_spectrum)
        energy1 = np.sum(magnitude_spectrum[ring1]) / total_energy
        energy2 = np.sum(magnitude_spectrum[ring2]) / total_energy
        energy3 = np.sum(magnitude_spectrum[ring3]) / total_energy
        energy4 = np.sum(magnitude_spectrum[ring4]) / total_energy
        
        # Enhanced deepfake detection
        mid_to_low_ratio = energy3 / (energy2 + 1e-10)
        high_to_mid_ratio = energy4 / (energy3 + 1e-10)
        
        # Stricter conditions for real faces
        is_real = (0.7 < mid_to_low_ratio < 1.3) and (0.5 < high_to_mid_ratio < 1.8)
        
        # Additional check: analyze phase information
        phase = np.angle(f_shift)
        phase_var = np.var(phase)
        is_real = is_real and phase_var > 1.0  # Deepfakes often have smoother phase
        
        confidence = min(0.5 + 0.3 * (1.0 - abs(mid_to_low_ratio - 1.0)) + \
                        0.2 * (1.0 - abs(high_to_mid_ratio - 1.0)) + \
                        0.2 * (phase_var / 2.0), 1.0)
        
        logger.info(f"Enhanced deepfake analysis: {'Real' if is_real else 'Fake'}, "
                    f"mid/low={mid_to_low_ratio:.2f}, high/mid={high_to_mid_ratio:.2f}, "
                    f"phase_var={phase_var:.2f}, confidence={confidence:.2f}")
...
def verify_liveness_multimethod(frame_sequence, transaction_amount=0):
    ...
    # Optical flow (critical for video replay detection)
    ...
    # Adjusted weights to prioritize robust methods
    weights = {
        'blink': 0.10,
        'head_movement': 0.10,
        'optical_flow': 0.30,  # Increased weight
        'texture': 0.20,       # Increased weight
        'frequency': 0.15,
        'deep_learning': 0.15,
        'deepfake': 0.20       # Increased weight
    }
    ...
    threshold = max(threshold, 0.8)  # Minimum threshold of 0.8
    is_live = final_score >= threshold
    
    # Require critical anti-spoofing methods to pass
...
def detect_screen_physical_properties(face_image):
    """Detect physical screen properties that cameras can't reproduce naturally"""
    try:
        # Convert to different color spaces
        hsv = cv2.cvtColor(face_image, cv2.COLOR_BGR2HSV)
        gray = cv2.cvtColor(face_image, cv2.COLOR_BGR2GRAY)
        
        # 1. Screen refresh rate artifacts (look for banding)
        # Apply FFT to detect periodic patterns
        fft = np.fft.fft2(gray)
        fft_shift = np.fft.fftshift(fft)
        magnitude = np.abs(fft_shift)
        
        # Look for periodic patterns characteristic of screens
        rows, cols = gray.shape
        center_row, center_col = rows // 2, cols // 2
        
        # Check for horizontal/vertical bands (screen refresh artifacts)
        ...
        # 2. Color temperature analysis (screens vs natural lighting)
        ...
        # 3. Pixel uniformity (screens have very uniform backlighting)
        brightness_uniformity = np.std(gray) / np.mean(gray)
        
        # 4. Edge sharpness (screens often have unnaturally sharp edges)
        ...
        # Screen indicators based on physical properties
        if h_variance > 10000 or v_variance > 10000:  # Periodic patterns
            screen_indicators += 1
            reasons.append(f"refresh_artifacts({h_variance:.0f},{v_variance:.0f})")
        
        if color_temp < 1.5 or color_temp > 3.0:  # Unnatural color temperature
            screen_indicators += 1
            reasons.append(f"unnatural_color_temp({color_temp:.2f})")
        
        if brightness_uniformity < 0.3:  # Too uniform (screen backlight)
            screen_indicators += 1
            reasons.append(f"too_uniform({brightness_uniformity:.3f})")
        
        if edge_intensity > 15:  # Unnaturally sharp edges
            screen_indicators += 1
...
def detect_screen_simple_no_edges(face_image):
    try:
        ...
        # Method 1: Texture uniformity
        laplacian_var = cv2.Laplacian(gray, cv2.CV_64F).var()
        
        # Method 2: Color histogram uniformity
...
def detect_phone_bezel(image):
    """Detect phone screen bezels/frames"""
    try:
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        
        # Look for rectangular shapes (phone screens)
        edges = cv2.Canny(gray, 50, 150)
        ...
        for contour in contours:
            ...
            if len(approx) == 4:
                ...
                # If large rectangular shape found (likely phone screen)
...
def detect_screen_simple_focused(face_image):
    """Backup detection method with even more relaxed thresholds"""
    try:
        ...
        # VERY STRICT - only flag extremely obvious cases
        # Based on your data, these should NEVER flag real faces
        
        extreme_indicators = 0
        
        # Only flag if saturation std is EXTREMELY low (much lower than your real face range)
        if sat_std < 60:  # Your real face is 73.3-75.5, so this is very safe
            extreme_indicators += 1
            logger.debug(f"Extreme indicator: very low sat std {sat_std:.1f}")
        
        # Only flag if variance is EXTREMELY high
        if color_variance > 4600:  # Your real face max is 4052, video close is 4403
            extreme_indicators += 1
            logger.debug(f"Extreme indicator: extremely high variance {color_variance:.1f}")
        
        # Combined extreme case
        if sat_mean < 110 and color_variance > 4400 and sat_std < 70:
            extreme_indicators += 1
            logger.debug(f"Extreme indicator: combined extreme case")
        
        # Only flag if we have multiple extreme indicators
...
def analyze_image_properties(face_image, label=""):
    """Detailed analysis for debugging screen detection"""
    try:
        hsv = cv2.cvtColor(face_image, cv2.COLOR_BGR2HSV)
        lab = cv2.cvtColor(face_image, cv2.COLOR_BGR2LAB)
...
def detect_moiré_patterns_simple(face_image):
    """Disabled moiré detection to prevent false positives"""
    return False, "Moiré detection disabled"
    
def enhanced_temporal_analysis_simple(frame_sequence):
    """More lenient temporal analysis to avoid false positives"""
    if len(frame_sequence) < 5:
    ...
    # Check compression artifacts (much more lenient)
    compression_scores = []
    ...
    if len(compression_scores) > 0:
        ...
        # MUCH MORE STRICT - only flag very obvious compression artifacts
        # Your real face shows compression mean=1097.7, so increase threshold significantly
        if comp_mean > 3000 and comp_std < 30:  # Much higher threshold
            logger.warning(f"Very obvious video compression detected")
            return True, 0.7, f"Strong video compression: mean={comp_mean:.1f}"
    
    # Check for very uniform frame differences (video loop)
    frame_diffs = []
    ...
    if len(frame_diffs) > 0:
        ...
        # Only flag very obvious looping patterns
        if diff_std < 1.0 and diff_mean < 3.0:  # Very strict
...
def hybrid_recognition(input_embedding, user_claim=None):
    ...
    if (user_count <= 5 or user_claim) and not force_svm:
        ...
        if prob >= 0.8:  # Reduced from 0.85 for real-world conditions
